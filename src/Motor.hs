...
frunGame :: GameContainer -> GameHistory -> IO ()
frunGame (GC bS players planktons (o:s)) (GH playersHistory planktonsHistory) = do 
  putStrLn ((stateToXyzFormat (players) (players)))
  --let playersAfterMoving = movePlayers players planktons bS
  --let newGC = processOverlappingCells bS playersAfterMoving planktons (o:s)
  --if equilibriumReached newGC then
  --  callOutputter (playersHistory++[getPlayers newGC]) (planktonsHistory++[getPlanktons newGC]) o
  --else
  --  frunGame newGC (GH (playersHistory++[getPlayers newGC]) (planktonsHistory++[getPlanktons newGC])) 
  
-- Arreglar la condicion de equilibrio
equilibriumReached :: GameContainer -> Bool
equilibriumReached gc = length (getPlayers gc) == 1

movePlayers :: [Cell] -> [Cell] ->  BoardSize -> [Cell]
